This document is the architecture and sizing reference for the AWS target. For the operator-facing deployment runbook, see [aws-production-operator-guide.md](./aws-production-operator-guide.md). The current W5 cloud-debugging target is a separate `hexacode-dev` stack; do not reuse or mutate existing `hexacode-prod` resources while validating W5 changes.
...
**2. Service Breakdown**
- `frontend`: React/Vite SPA. Public routes are `/`, `/problems`, `/problems/{slug}`, `/problems/{slug}/solve`, `/submissions`, `/submissions/{id}`, `/login`, `/signup`, `/forgot-password`, `/new-password`, `/auth/callback`, and `/dashboard/*` for problem, tag, user, operations, and storage management. Public API calls include `/api/problems`, `/api/problems/{slug}/solve`, `/api/tags`, `/api/runtimes`, `/api/auth/me`, `/api/submissions*`, `/api/submissions/problem-states`, `/api/chat/messages`, and dashboard routes via the gateway. Auth is direct browser-to-Cognito using the Cognito Identity Provider SDK; the SPA stores tokens in localStorage and sends the Cognito `idToken` as the bearer token, falling back to `accessToken` if the ID token is absent.
...
- `submission-service`: Public APIs are `/api/runtimes`, `/api/submissions`, `/api/submissions/problem-states`, `/api/submissions/{id}`, `/api/submissions/{id}/source`, `/api/submissions/{id}/results`, `/api/submissions/{id}/files/{object_id}`, and `/api/dashboard/operations`. Internal APIs are `/internal/runtimes/{profile_key}`, `/internal/judge-jobs/{job_id}/context`, `/started`, and `/completed`. Dependencies are PostgreSQL, SQS, Cognito JWKS, internal HTTP call to problem-service for cache invalidation, and submission artifact storage through S3-compatible or EFS-backed storage. Logical ownership is `submission.*`; it also updates `problem.problem_stats`.
- `worker`: No public API. Polls SQS, fetches submission context from submission-service, fetches problem judge context from problem-service, optionally fetches checker runtime context, runs compile/execute locally in the container, uploads compiled custom checker artifacts to the problems bucket, persists stdout/stderr/compile-log artifact descriptors through the configured storage driver, and posts started/completed callbacks to submission-service. Dependencies are SQS, internal HTTP to problem/submission services, S3, and EFS when `STORAGE_DRIVER=efs`. The Dockerfile installs `g++`; execution is direct subprocess execution inside the container.
- `chat-lambda`: No database ownership. Accepts `POST /api/chat/messages` in Lambda proxy form, validates the session/message/page-context payload, invokes Bedrock Agent Runtime with `BEDROCK_AGENT_ID` and `BEDROCK_AGENT_ALIAS_ID`, optionally passes knowledge-base retrieval configuration, and returns a Lambda proxy response containing the frontend API envelope. In cloud it sits behind real AWS API Gateway; in local dev the thin gateway invokes it directly to preserve the same request/response contract.
...
- Submission create flow: `submission-service` validates runtime/problem/testset inputs, inserts `submission.submissions`, inserts `submission.judge_jobs`, inserts `submission.outbox_events`, persists submitted source at `submissions/{submission_id}/source/source.txt`, then publishes one SQS message.
...
- Worker completion contract: worker calls `/internal/judge-jobs/{job_id}/completed` with `worker_name`, `status_code`, `verdict_code`, `runtime_ms`, `memory_kb`, optional `compile_log_artifact` descriptor or `compile_log_object_id`, `compile_exit_code`, `compile_time_ms`, and a `results[]` array. Each result is `compile`, `testcase`, or `custom_case`; testcase results may carry previews plus `stdout_artifact`/`stderr_artifact` descriptors or existing `stdout_object_id`/`stderr_object_id` references.
...
- Storage contract: `storage.objects` stores metadata: `bucket`, `object_key`, `content_type`, `original_filename`, `size_bytes`, `sha256`, `etag`, `metadata_json`, `storage_driver`, `filesystem_path`, and `artifact_kind`, with timestamps maintained by the schema. `filesystem_path` is populated for EFS-backed artifacts; binary payloads still stay outside PostgreSQL.
- Object key patterns actually used in code: `problem/{problem_id}/statement/{object_id}.{ext}`, `problem/{problem_id}/media/{object_id}-{filename}`, `testset/{testset_id}/archive/{object_id}.{ext}`, `testset/{testset_id}/cases/{ordinal}/input.{ext}`, `testset/{testset_id}/cases/{ordinal}/output.{ext}`, `problem/{problem_id}/checker/{object_id}.{ext}`, `problem/{problem_id}/checker/{checker_id}/compiled/{sha256}-{artifact}`, `submissions/{submission_id}/source/source.txt`, and worker artifact keys under `submissions/{submission_id}/...`.
...
| `chat-lambda` | AWS Lambda behind API Gateway HTTP API | Chat is already implemented as a Lambda proxy handler that calls Bedrock and does not need to transit a containerized FastAPI service. Provisioned concurrency (default 2) eliminates cold starts; API Gateway invokes the "live" alias. |
...
Internet
  ...
  |      |
  |      +--> Lambda: chat-lambda -> Bedrock Agent Runtime
  |
...
3. Use the Terraform-managed ECR repository from `ecr_repository_name` for backend images. For the W5 dev stack, use `dev/hexacode`; do not push new W5 debug images into `prod/hexacode`.
4. Build and push backend images into the environment-specific ECR repository and distinguish them with service-prefixed tags such as `identity-service-dev-<gitsha>` and `problem-service-dev-<gitsha>` for dev. `identity-service`, `submission-service`, and `worker` build with `hexacode-backend` as context; `problem-service` still uses the repo root as Docker build context so its image can include the curated `data/problems` catalog.
...
8. Create API Gateway HTTP API with one VPC Link to the internal ALB. Mirror the route manifest prefixes as API Gateway routes, enable CORS for the frontend domain, and enable access logs. The `$default` stage applies default throttling (200 burst / 100 rate) to all routes.
9. Add a dedicated `POST /api/chat/messages` route in API Gateway HTTP API that integrates directly with the deployed `chat-lambda` alias (not the unqualified function ARN) instead of forwarding that route through the internal ALB. This route has stricter throttling (20 burst / 10 rate). The chat Lambda uses provisioned concurrency (default 2 instances) to eliminate cold starts for Bedrock agent calls.
...
- Dev/prod isolation is mandatory for W5 validation. The dev stack should use `environment = "dev"`, a non-overlapping VPC CIDR, `dev/terraform.tfstate`, `dev/hexacode` ECR, and `hexacode-dev-*` resource names. Stop any deployment plan that references existing `hexacode-prod` resources.
- The W5 security controls are Terraform-managed and environment-scoped: WAF, Network Firewall, VPC Flow Logs, EFS, AWS Backup, Cognito/JWT API Gateway auth, chat throttling, and Lambda reserved concurrency should all appear with the target environment suffix in the reviewed plan.
...
- Firewall route tables: `0.0.0.0/0 -> nat`
- Private app/data route tables: `0.0.0.0/0 -> per-AZ Network Firewall endpoint` when the firewall module is enabled; direct NAT routes are used only when `network_firewall_enabled` is false.
...
- `sg_worker`: inbound none; outbound `tcp/80` to `sg_internal_alb`; outbound `tcp/443` to `0.0.0.0/0`; outbound `tcp/2049` to `sg_efs`.
- `sg_efs`: inbound `tcp/2049` from `sg_submission_service`, `sg_worker`, and the SSM management host security group when Management VPC is enabled.
...
- `ecs_task_role_submission`: trust `ecs-tasks.amazonaws.com`; `sqs:GetQueueUrl`,`sqs:GetQueueAttributes`,`sqs:SendMessage` on the judge queue ARN; `s3:ListBucket`,`s3:GetBucketLocation` on `hexacode-${env}-submission-artifacts`; `s3:GetObject` on `arn:aws:s3:::hexacode-${env}-submission-artifacts/*`; `elasticfilesystem:ClientMount` and `elasticfilesystem:ClientWrite` on the artifact EFS file system constrained to the configured access point.
- `ecs_task_role_worker`: trust `ecs-tasks.amazonaws.com`; `sqs:GetQueueUrl`,`sqs:GetQueueAttributes`,`sqs:ReceiveMessage`,`sqs:DeleteMessage`,`sqs:ChangeMessageVisibility` on the judge queue ARN; `s3:GetObject` on `arn:aws:s3:::hexacode-${env}-problem-assets/problem/*` and `arn:aws:s3:::hexacode-${env}-problem-assets/testset/*`; `s3:PutObject` on `arn:aws:s3:::hexacode-${env}-problem-assets/problem/*/checker/*/compiled/*`; `elasticfilesystem:ClientMount` and `elasticfilesystem:ClientWrite` on the artifact EFS file system constrained to the configured access point. Add `s3:GetObject` on the submission bucket later if the worker starts reading submission artifacts from S3.
...
- `hexacode-${env}-submission-artifacts`: private bucket, same encryption/public-block posture. Submission-service can persist source files at `submissions/{submission_id}/source/source.txt`; current Terraform sets `STORAGE_DRIVER=efs` for submission-service and worker so runtime source/stdout/stderr/compile-log artifacts are primarily written through the EFS-backed artifact path.
- Access pattern: frontend bucket read by CloudFront only; problem bucket read/write by problem-service and worker; submission artifact records are owned by submission-service, with worker returning artifact descriptors for persisted execution outputs.
- Lifecycle: no expiry for statements/testcases; optionally expire `problem/*/checker/*/compiled/*` after 90 days; do not add destructive expiry to submission artifacts until the product decides retention for source/log/stdout/stderr artifacts.
...
- Environment/secrets injection:
  ...
  - EFS artifact storage: Terraform sets `STORAGE_DRIVER=efs`, `ARTIFACT_STORAGE_ROOT=/mnt/submission-artifacts`, mounts the EFS access point into submission-service and worker, and keeps transit encryption enabled
