...
4. Build and push backend images from `hexacode-backend/`. Store all backend services in the environment-specific ECR repository and distinguish them with service-prefixed tags such as `identity-service-dev-<gitsha>` and `problem-service-dev-<gitsha>` for dev. The catalog import helper now lives under the backend workspace as `hexacode-backend/scripts/import_problem_catalog.py`, so `problem-service` no longer needs repo-root build context.
